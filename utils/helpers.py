    ...
    try:
        query = """
            SELECT
                ...
                v.consultor_ist,
                COALESCE(u.cargo, v.consultor_cargo) AS consultor_cargo,
                COALESCE(u.zonal, v.consultor_zonal) AS consultor_zonal,
                v.note_visita,
                v.tipo_evaluacion,
                u.name AS consultor_nombre,
                u.name_complete AS consultor_name_complete,
                ec.temperatura_dia,
                ec.equipo_temp,
                ec.equipo_vel_air,
                ec.patron_tbs,
...
def guardar_visita_inicio(visita_data, confort_data=None, ventilacion_data=None):
    """Inserta una visita y, de ser necesario, sus datos específicos por tipo."""
    db = MySQLDatabaseManager()
    try:
...
def actualizar_visita_inicio(id_visita, visita_data, confort_data=None, ventilacion_data=None):
    """Actualiza la información base de una visita y sus datos específicos."""
    db = MySQLDatabaseManager()
    try:
        consultor_info = _obtener_datos_consultor(db, visita_data["consultor_ist"])
        if not consultor_info:
